...
function Header()
{
    ...
    $this->Cell(0,10,'Reporte de Gastos Extraordinarios',0,0,'C');
    $this->Ln(15);

    // Centrar los encabezados de la tabla
    $this->SetX(($this->GetPageWidth() - 235) / 2);
...
require ("cn.php");
$consulta = "SELECT * FROM `gastos-extraordinarios`";
...
while ($row=$resultado->fetch_assoc()) {
    $pdf->SetX(($pdf->GetPageWidth() - 235) / 2); // Ajusta el margen izquierdo dinámicamente
    $pdf->Cell(40,6,$row['codigo'],1,0,'C');
    $pdf->Cell(40,6,$row['id-casa'],1,0,'C');
    $pdf->Cell(30,6,$row['nombre'],1,0,'C');
    $pdf->Cell(40,6,$row['monto'],1,0,'C');
    $pdf->Cell(40,6,$row['fecha'],1,0,'C');
    $pdf->Cell(45,6,$row['estado'],1,1,'C');
}
